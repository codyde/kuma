...
var _ = Describe("kumactl install kong-ingress", func() {
	...
	DescribeTable("should generate Kubernetes resources",
		func(given testCase) {
			...
			rootCmd.SetArgs(append([]string{"install", "kong-ingress"}, given.extraArgs...))
		...
		Entry("should generate Kubernetes resources with default settings", testCase{
			extraArgs:  nil,
			goldenFile: "install-kong-ingress.defaults.golden.yaml",
		}),
		Entry("should generate Kubernetes resources with custom settings", testCase{
			extraArgs: []string{
				"--namespace", "kuma",
			},
			goldenFile: "install-kong-ingress.overrides.golden.yaml",
		}),
	)
})
